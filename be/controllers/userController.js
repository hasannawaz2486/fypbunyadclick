...
exports.registerUser = catchAsyncErrors(async (req, res, next) => {
  ...
  const { name, email, password, confirmPassword, role, address, phoneNo, country , state ,city, store, aboutInfo } = req.body

  console.log(req.body)
  if (!name || !email || !password || !confirmPassword || !role || !phoneNo || !city  || !country || !state) {
...
//   remove the filed that are empty
    Object.keys(user).forEach((key) => (user[key] === undefined || user[key] == "") && delete user[key])
